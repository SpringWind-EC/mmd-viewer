...
type MotionKeyframe = {
  ...
  rotationModes?: Record<string, "absolute" | "delta">;
};
...
const neutralQuaternion: QuaternionArray = [0, 0, 0, 1];
...
export class MotionPlayer {
  ...
  private applyBoneRotations(
    ...
    for (const boneName of boneNames) {
      ...
      const rotationMode = this.rotationModeForBone(kfA, kfB, boneName);
      const a = this.rotationForKeyframe(kfA, boneName, rotationMode);
      const b = this.rotationForKeyframe(kfB, boneName, rotationMode);
  ...
  private applyBonePositions(
    ...
    for (const boneName of boneNames) {
      ...
      const useAbsolute = this.motion?.positionMode === "absolute";
      const posA = new THREE.Vector3(
        (useAbsolute ? 0 : restPos.x) + a[0],
        (useAbsolute ? 0 : restPos.y) + a[1],
        (useAbsolute ? 0 : restPos.z) + a[2]
      );

      const posB = new THREE.Vector3(
        (useAbsolute ? 0 : restPos.x) + b[0],
        (useAbsolute ? 0 : restPos.y) + b[1],
        (useAbsolute ? 0 : restPos.z) + b[2]
      );
  ...
  private rotationForKeyframe(
    keyframe: MotionKeyframe,
    boneName: string,
    rotationMode: "absolute" | "delta"
  ): QuaternionArray | null {
    const keyed = keyframe.bones?.[boneName];
    ...
    return this.quaternionToArray(this.restPose[boneName]);
  }

  private rotationModeForBone(
    kfA: MotionKeyframe,
    kfB: MotionKeyframe,
    boneName: string
  ): "absolute" | "delta" {
    return (
      kfB.rotationModes?.[boneName] ??
      kfA.rotationModes?.[boneName] ??
      this.motion?.rotationMode ??
      "absolute"
    );
  }
}
