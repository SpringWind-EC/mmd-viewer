...
export function peaceSignPose(side: "right" | "left"): BoneMap {
  const isRight = side === "right";
  const prefix = isRight ? "right" : "left";
  ...
  return mergeBones(handPose(side, "fist"), {
    [bone("Index1")]: axisQuat(0, sign * 0.04, sign * 0.16),
    [bone("Index2")]: q(RigCalibration.hand.openFinger[side]),
    [bone("Index3")]: q(RigCalibration.hand.openFinger[side]),
    [bone("Middle1")]: axisQuat(0, -sign * 0.03, -sign * 0.12),
    [bone("Middle2")]: q(RigCalibration.hand.openFinger[side]),
    [bone("Middle3")]: q(RigCalibration.hand.openFinger[side]),
  });
}

export function bothPeaceSignsPose(): BoneMap {
  return mergeBones(peaceSignPose("right"), peaceSignPose("left"));
}
